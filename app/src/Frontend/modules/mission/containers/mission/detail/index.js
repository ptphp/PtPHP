...
import VerifyView from '../verify/index';
...
export default React.createClass( {
    ...
    renderTaskCtl(task){
        let {user_mission,user_missions,mission} = this.state;
    ...
    renderDetail(){
        ...
        if(this.state.missionBtnStatus == 0){
            if(this.state.mission.remain_times == 0){
                top_ctl = <span className="join_btn has_join">任务参与人数已满</span>;
            }else if(this.state.mission.tips1){
                top_ctl = <span className="join_btn has_join">{this.state.mission.tips1}</span>;
            }else{
                top_ctl = <span className="join_btn do_join" onClick={this.onBeginTask}>参与</span>;
            }
        }else if(this.state.missionBtnStatus == 1){
            top_ctl = <span className="join_btn has_join">已报名</span>;
        }else{
            top_ctl = <span className="join_btn has_join">已完成</span>;
        }
            ...
            <Panel>
                <PanelBody>
                    <MediaBox type="appmsg">
                        ...
                        <MediaBoxBody>
                            <MediaBoxTitle>{mission.platform_name}</MediaBoxTitle>
                            <MediaBoxDescription >
            ...
            <Article className="Article" style={{paddingLeft:24,paddingTop:16,paddingBottom:0}}>
                <section className="xiandingwei">
                    {this.state.tasks.map(task=>{
                        ...
                        if(this.state.user_mission && task.key == this.state.user_mission.task_key){
                            if(this.state.user_mission.verify_id){
                                let verify = this.getVerifyDetail(this.state.user_mission.verify_id);
                                if(verify.status == 1 ){
                                    class_name = "TaskItem Articleheight finish";
                                }
                            }
                        }
                        return (
    ...
    render() {
        ...
        if(this.state.showVerifyModal){
            return (
                <Modal title="提交审核" onClose={this.hideVerifyModal} show={this.state.showVerifyModal}>
                    <VerifyView parent={this}/>
                </Modal>
            );
        }else if(this.state.showExampleModal){
            return (
                <Modal title="攻略" onClose={this.hideExampleModal} show={this.state.showExampleModal}>
                    <ExampleView parent={this}/>
                </Modal>
            );
        }else{
            return (
