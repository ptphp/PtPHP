...
// 	TODO maybe window.addEventListener('offline', this.updateOnlineStatus);
    ...
    this.apiQueue = async.queue(function (opts, callback) {
        ...
        httpify({
            ...
            if(SHOW_DEV_TAB) console.log(err,res,res.body)
            //console.log(res)
            if (res.statusCode !== 200) return callback(new Error('Error ' + res.statusCode));

            var body = res.body || {};
            if (body.status > 0) return callback(new Error('API Error: ' + body.message));

            var data = body.return || {};
...
DataStore.prototype.amRegistered = function () {
    return !!this.cache.authToken
};
DataStore.prototype.getAttendees = function () {
    return this.cache.people.filter(person => !person.isOrganiser && !person.isSpeaker)
};
DataStore.prototype.getMe = function () {
    return this.cache.me
};
DataStore.prototype.getOrganisers = function () {
    return this.cache.people.filter(person => person.isOrganiser)
};
DataStore.prototype.getPerson = function (id) {
    return this.cache.people.filter(person => person.id === id).pop()
};
DataStore.prototype.getPeople = function () {
    return this.cache.people
};
DataStore.prototype.getSchedule = function () {
    return this.cache.schedule.sort((a, b) => new Date(a.startTime) - new Date(b.startTime))
};
DataStore.prototype.getSettings = function () {
    return this.cache.settings
};
DataStore.prototype.getSpeakers = function () {
    return this.cache.people.filter(person => person.isSpeaker)
};
DataStore.prototype.getSponsors = function () {
    return this.cache.sponsors
};
DataStore.prototype.getTicketCode = function () {
    return this.cache.ticketCode
};

//消息列表
DataStore.prototype.message_list = function (callback) {
    this.apiQueue.push(
        {
            endpoint: 'controller=admin/weixin/api/message&action=list'
        },
        callback ||
        function () {}
    );
...
//消息详情
DataStore.prototype.message_detail = function (id,callback) {
    this.apiQueue.push(
        {
            endpoint: 'controller=admin/weixin/api/message&action=detail',
            data:{id:id}
        },
        callback ||
        function () {}
...
//日报创建
DataStore.prototype.daily_create = function (info,callback) {
    this.apiQueue.push(
        {
            endpoint: 'controller=admin/weixin/api/daily&action=create',
            data:{info:info}
        },
        callback ||
        function () {}
...
//日报评论
DataStore.prototype.daily_commnet_add = function (info,callback) {
    this.apiQueue.push(
        {
            endpoint: 'controller=admin/weixin/api/daily&action=comment_add',
            data:{info:info}
        },
        callback ||
        function () {}
...
//日报评论
DataStore.prototype.daily_commnet_list = function (info,callback) {
    this.apiQueue.push(
        {
            endpoint: 'controller=admin/weixin/api/daily&action=comment_add',
            data:{info:info}
